...
public class SysMenuController extends BaseController {
    ...
    private static final  int  MAX_SYS_MENU_ID=31;
    ...
    public List<SysMenuEntity> list() {
        ...
        for (SysMenuEntity sysMenuEntity : menuList) {
            ...
            if (parentMenuEntity != null&&parentMenuEntity.getDelFlag()==0) {
    ...
    public AjaxResult info(@PathVariable("menuId") Long menuId) {
        ...
        return AjaxResult.ok().put("menu", menu);
    }
    ...
    public AjaxResult save(@RequestBody SysMenuEntity menu) {
        //数据校验
        verifyForm(menu);
    ...
    public AjaxResult update(@RequestBody SysMenuEntity menu) {
        //数据校验
        verifyForm(menu);
    ...
    public AjaxResult delete(long menuId) {
        if (menuId <= MAX_SYS_MENU_ID) {
            return AjaxResult.error("系统菜单，不能删除");
        }
        //判断是否有子菜单或按钮
        List<SysMenuEntity> menuList = sysMenuService.queryListParentId(menuId);
        if (menuList.size() > 0) {
            return AjaxResult.error("请先删除子菜单或按钮");
        }
